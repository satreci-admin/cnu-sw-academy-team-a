...
public class SSH_Controller {
    ...
    //Constructor
    public SSH_Controller(){
        //(1) set_localHostUser : 로컬 서버의 계정 이름을 저장한다.
        this.set_localHostUser(System.getProperty("user.name"));

        //(2) set_localHostIP : 시스템에서 localHost 의 ip 주소를 찾아 저장한다. //this local host 168.188.129.195

        /*
        try {
            InetAddress ip;
            ip = InetAddress.getLocalHost();
            this.set_localHostIp(ip.getHostAddress());
        }
